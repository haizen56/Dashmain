                ...
                <div className="flex-col overflow-hidden md:overflow-auto justify-start items-start gap-4 flex bg-zinc-900 px-4 py-6 min-h-full">
                    {
                        Data.map((icon, index,) => {
                            return (
                                <div key={index} onClick={(r) => { ClickedIcon(icon) }} className={`p-3.5 cursor-pointer ${activeIcon === icon.icon ? ' text-white bg-gradient-to-bl from-amber-200 to-pink-600' : 'text-neutral-400'}  rounded-lg flex-col hover:text-white duration-300 justify-start items-center gap-2 flex`}>
                        ...
                        category.map(({ text, icon, url }, index) => {
                            return (
                                <Link to={`/admin-dashboard/${url}`} onClick={() => {
                                    setActiveCat(text)
                                }} key={index} className={`self-stretch duration-300 cursor-pointer px-[18px] ${activeCat === text ? ' bg-orange-50 bg-opacity-80 border-r-2 border-pink-600 ' : 'text-zinc-500 hover:bg-neutral-200'} py-3.5 justify-start items-center gap-3 inline-flex`}>
                                    <div className={`w-6 relative ${activeCat === text ? 'text-pink-600' : 'text-zinc-500'} text-xl`} ><i className={`${icon}`}></i></div>
                                    <div className={`grow shrink basis-0 ${activeCat === text ? 'text-pink-600 font-semibold' : 'text-zinc-500  font-normal'} text-sm leading-tight`}>{text}</div>
                                </Link>
                            )
                        })
                    }
